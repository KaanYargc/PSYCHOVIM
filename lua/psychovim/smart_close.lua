...
local function modified_buffer_count()
  ...
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
    if vim.api.nvim_buf_is_valid(buf) and vim.bo[buf].buflisted and vim.bo[buf].modified then count = count + 1 end
  end
  return count
end
...
local function quit_without_saving() close_popup(); vim.cmd("qa!") end
...
function M.open()
  ...
  state.win = vim.api.nvim_open_win(state.buf, true, {
    ...
    style = "minimal", border = "rounded", title = " PychoClose ", title_pos = "center", noautocmd = true,
  })
